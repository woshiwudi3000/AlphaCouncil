...
*   **🚀 多模型协同 (Model Agnostic)**：支持混合调度 **通义千问 (Qwen)** 和 **DeepSeek-R1 (Reasoner)** 模型。
...
*   **Qwen**：通过标准 REST API (`fetch`) 调用，支持 OpenAI 兼容格式，用于宏观、行业、资金流向分析。
*   **DeepSeek**：通过标准 REST API (`fetch`) 调用，支持 OpenAI 兼容格式，用于技术分析、基本面估值、总监整合、风控评估、总经理决策。
*   **Fallback 机制**：如果首选模型失败（如 key 错误），系统会自动降级使用 Qwen 模型完成分析，确保流程不中断。
...
3. **配置环境变量（强烈推荐）**
   ...
   GEMINI_API_KEY=你的_Gemini_API_密钥（可选）
   ```
